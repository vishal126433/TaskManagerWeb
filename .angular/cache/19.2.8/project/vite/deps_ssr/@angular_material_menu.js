...
import {
  MatRippleModule
} from "./chunk-3J4I6JDD.js";
import {
  DomPortalOutlet,
  PortalModule,
  TemplatePortal
} from "./chunk-LU5JBUCV.js";
import {
  MatRipple,
...
var MatMenuItem = class _MatMenuItem {
  ...
  _hovered = new import_rxjs3.Subject();
  /** Stream that emits when the menu item is focused. */
  _focused = new import_rxjs3.Subject();
...
var MatMenuContent = class _MatMenuContent {
  ...
  _attached = new import_rxjs3.Subject();
  constructor() {
  }
  /**
...
var MatMenu = class _MatMenu {
  ...
  ngAfterContentInit() {
    ...
    this._directDescendantItems.changes.pipe((0, import_operators3.startWith)(this._directDescendantItems), (0, import_operators3.switchMap)((items) => (0, import_rxjs3.merge)(...items.map((item) => item._focused)))).subscribe((focusedItem) => this._keyManager.updateActiveItem(focusedItem));
  ...
  _hovered() {
    ...
    return itemChanges.pipe((0, import_operators3.startWith)(this._directDescendantItems), (0, import_operators3.switchMap)((items) => (0, import_rxjs3.merge)(...items.map((item) => item._hovered))));
  }
  /*
...
var MatMenuTrigger = class _MatMenuTrigger {
  ...
  _overlayRef = null;
  _menuOpen = false;
  _closingActionsSubscription = import_rxjs3.Subscription.EMPTY;
  _hoverSubscription = import_rxjs3.Subscription.EMPTY;
  _menuCloseSubscription = import_rxjs3.Subscription.EMPTY;
  _pendingRemoval;
  /**
  ...
  _destroyMenu(reason) {
    ...
    if (menu instanceof MatMenu && this._ownsMenu(menu)) {
      this._pendingRemoval = menu._animationDone.pipe((0, import_operators3.take)(1)).subscribe(() => {
        overlayRef.detach();
  ...
  _menuClosingActions() {
    ...
    const parentClose = this._parentMaterialMenu ? this._parentMaterialMenu.closed : (0, import_rxjs3.of)();
    const hover = this._parentMaterialMenu ? this._parentMaterialMenu._hovered().pipe((0, import_operators3.filter)((active) => this._menuOpen && active !== this._menuItemInstance)) : (0, import_rxjs3.of)();
    return (0, import_rxjs3.merge)(backdrop, parentClose, hover, detachments);
  }
